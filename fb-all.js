/*1393523353,182616881,JIT Construction: v1139846,en_US*/
/**
...
try {
    window.FB || (function (window) {
        ...
        __d("URL", ["Assert", "copyProperties", "QueryString", "Log"], function (a, b, c, d, e, f, g, h, i, j) {
            var k = new RegExp('(' + '(((\\w+):)?//)' + '(.*?@)?' + '([^~/?#:]+)' + '(:(\\d+))?' + ')?' + '([^\\?$#]+)?' + '(\\?([^$#]+))?' + '(#([^$]+))?'),
                l = /[\0\\]/,
                m = /[^\w\-\.,;\/?:@=&%#$~+!*'\[\]()]+/g,
        ...
        __d("sdk.XD", ["sdk.Content", "sdk.Event", "Log", "QueryString", "Queue", "sdk.RPC", "sdk.Runtime", "sdk.Scribe", "URL", "UrlMap", "XDConfig", "XDM", "sdk.createIframe", "sdk.feature", "guid"], function (a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u) {
            var v = new k(),
                w = new k(),
                x = new k(),
                y, z, aa = u(),
                ba = u(),
                ca = location.protocol + '//' + location.host,
                da, ea = false,
                fa = 'Facebook Cross Domain Communication Frame',
                ga = {}, ha = new k();
            l.setInQueue(ha);

            function ia(oa) {
                i.info('Remote XD can talk to facebook.com (%s)', oa);
                m.setEnvironment(oa === 'canvas' ? m.ENVIRONMENTS.CANVAS : m.ENVIRONMENTS.PAGETAB);
            }
            ...
            function ja(oa, pa) {
                ...
                case 'proxy_ready':
                    ...
                    i.info('Proxy ready, starting queue %s containing %s messages', qa + 'ProxyQueue', ra.getLength());
                    ra.start(function (ua) {
                        da.send(typeof ua === 'string' ? ua : j.encode(ua), pa, sa.contentWindow, ba + '_' + qa);
                    });
                    break;
                case 'plugin_ready':
                    i.info('Plugin %s ready, protocol: %s', oa.name, qa);
                    ga[oa.name] = {
                        protocol: qa
                    };
                    if (k.exists(oa.name)) {
                        var ta = k.get(oa.name);
                        i.debug('Enqueuing %s messages for %s in %s', ta.getLength(), oa.name, qa + 'ProxyQueue');
                        (qa == 'https' ? x : w).merge(ta);
                    }
                    break;
                }
            ...
            function ka(oa, pa) {
                if (pa && pa !== 'native' && !o(pa).isFacebookURL()) return;
                if (typeof oa == 'string') {
                    if (/^FB_RPC:/.test(oa)) {
                    ...
                    if (oa.substring(0, 1) == '{') {
                        try {
                            ...
                            i.warn('Failed to decode %s as JSON', oa);
                            return;
                        }
                    } else oa = j.decode(oa);
                }
                if (!pa)
                    if (oa.xd_sig == aa) pa = oa.xd_origin;
                if (oa.xd_action) {
                    ja(oa, pa);
                    return;
                }
                if (oa.access_token) m.setSecure(/^https/.test(ca));
                if (oa.cb) {
            ...
            function la(oa, pa) {
                if (oa == 'facebook') {
                    ...
                    var qa = ga[oa];
                    if (qa) {
                        i.debug('Enqueuing message for plugin %s in %s', oa, qa.protocol + 'ProxyQueue');
                        (qa.protocol == 'https' ? x : w).enqueue(pa);
                    } else {
                        i.debug('Buffering message for plugin %s', oa);
                        k.get(oa).enqueue(pa);
                    }
                }
            }
            l.getOutQueue().start(function (oa) {
                la('facebook', 'FB_RPC:' + oa);
            });

            function ma(oa) {
                if (ea) return;
                var pa = g.appendHidden(document.createElement('div')),
                    qa = r.create({
                        blacklist: null,
                        root: pa,
                        channel: ba,
                        flashUrl: q.Flash.path,
                        whenReady: function (ra) {
                            da = ra;
                            var sa = {
                                channel: ba,
                                ...
                                transport: qa,
                                xd_name: oa
                            }, ta = q.XdUrl + '#' + j.encode(sa),
                                ua = q.useCdn ? 'cdn' : 'www',
                                va = p.resolve(ua, false),
                                wa = p.resolve(ua, true);
            ...
            var na = {
                ...
                handler: function (oa, pa, qa, ra) {
                    var sa = location.protocol == 'https:',
                        ta = q.useCdn ? 'cdn' : 'www',
                        ua = p.resolve(ta, sa);
                    return ua + q.XdUrl + '#' + j.encode({
            ...
            h.subscribe('init:post', function (oa) {
                ma(oa.xdProxyName);
                var pa = t('xd_timeout');
                if (pa) setTimeout(function () {
                    var qa = z && ( !! y == w.isStarted() && !! z == x.isStarted());
                    if (!qa) n.log('jssdk_error', {
                        appId: m.getClientID(),
                        error: 'XD_INITIALIZATION',
                        extra: {
                            message: 'Failed to initialize in ' + pa + 'ms'
                        }
                    });
                }, pa);
            });
        ...
        __d("sdk.Auth", ["sdk.Cookie", "copyProperties", "sdk.createIframe", "DOMWrapper", "sdk.feature", "sdk.getContextType", "guid", "sdk.Impressions", "Log", "ObservableMixin", "QueryString", "sdk.Runtime", "sdk.SignedRequest", "UrlMap", "URL", "sdk.XD"], function (a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u, v) {
            var w, x, y = new p();
        ...
        __d("mergeInto", ["mergeHelpers"], function (a, b, c, d, e, f, g) {
            "use strict";
            var h = g.checkMergeObjectArg;
        ...
        __d("merge", ["mergeInto"], function (a, b, c, d, e, f, g) {
            "use strict";
            var h = function (i, j) {
                var k = {};
                g(k, i);
        ...
        __d("ApiClient", ["ArgumentError", "Assert", "copyProperties", "CORSRequest", "FlashRequest", "flattenObject", "JSONPRequest", "Log", "merge", "ObservableMixin", "sprintf", "URL", "UrlMap", "ApiClientConfig"], function (a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t) {
            var u, v, w, x = {
                    get: true,
                    post: true,
                    'delete': true,
    ...
    new Image().src = "https:\/\/www.facebook.com\/" + 'common/scribe_endpoint.php?c=jssdk_error&m=' + encodeURIComponent('{"error":"LOAD", "extra": {"name":"' + e.name + '","line":"' + (e.lineNumber || e.line) + '","script":"' + (e.fileName || e.sourceURL || e.script) + '","stack":"' + (e.stackTrace || e.stack) + '","revision":"1139846","message":"' + e.message + '"}}');
}