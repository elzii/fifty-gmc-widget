###
Global Moms Challenge Widget
Copyright Fifty and Fifty, LLC.

Licensed under the Apache License, Version 2.0
http://www.apache.org/licenses/LICENSE-2.0
###
try
  
  # set vars
  request = null
  debug = true
  tag_id = "gmc_widget"
  
  # Append iFrame and such
  window.onload = ->
    
    # set script/scripts var
    scripts = document.getElementsByTagName("script")
    script = undefined
    i = 0

    while i < scripts.length
      if scripts[i].getAttribute("rel") is "gmc"
        script = scripts[i]
        break
      i++
    if script
      
      # get parent node of <script> el
      script_parent_el = script.parentNode
      
      # create iframe
      gmc_widget = document.createElement("iframe")
      gmc_widget.setAttribute "id", tag_id
      
      # create gmc widget styles
      gmc_widget_styles = document.createElement("style")
      gmc_widget_styles.v = 0
      gmc_widget_styles.setAttribute "id", "" + tag_id + "_styles"
      
      # insert the gmc_widget element near the script tag's position in body
      script_parent_el.insertBefore gmc_widget
      
      # IE8+ & Modern Browsers
      #////////////////////////////////////////////////////////////////////////////////////////////////
      if typeof window.ActiveXObject is "undefined"
        request = new XMLHttpRequest
        request.open "GET", "https://s3.amazonaws.com/gmc_widget/gmc_numbers/dev_numbers.json", true
        request.onload = ->
          if request.status >= 200 and request.status < 400
            
            # Success!
            
            # set data in various forms (raw, string, object)
            data = JSON.parse(request.responseText)
            
            # remove quotes from css in JSON
            gmc_widget_styles_raw = data.site_info.css.replace(/\"/g, "")
            gmc_widget.contentWindow.document.write "<style>" + gmc_widget_styles_raw + "</style>"
            
            # get width/height values for iframe set as data attr in <script> tag
            gmc_widget_width = script.getAttribute("data-width")
            gmc_widget_height = script.getAttribute("data-height")
            
            # log gmc.js width/height values
            if debug
              console.log "-------------------------------------------------------------"
              console.log "gmc_widget_width: ", gmc_widget_width
              console.log "gmc_widget_height: ", gmc_widget_height
            
            # set default styles for gmc_widget iframe (remove border and outline)
            gmc_widget.style.border = 0
            gmc_widget.style.outline = 0
            gmc_widget.contentWindow.document.write "<style>body{margin:0;padding:0;}</style>"
            
            # log gmc_widget iframe styles
            if debug
              console.log "-------------------------------------------------------------"
              console.log gmc_widget.style
            
            # log gmc_widget values
            if debug
              console.log "-------------------------------------------------------------"
              console.log "gmc_widget: ", gmc_widget
              console.log "gmc_widget: ", gmc_widget.contentWindow.document
            
            # build out the HTML string to put in #gmc_widget
            #/////////////////////////////////////////////////////////////////////////////////////////////////
            
            # 300x600
            #            ================================================== 
            if (gmc_widget_width is "300" and gmc_widget_height is "250") or gmc_widget_width is `undefined`
              gmc_widget.style.width = "300px"
              gmc_widget.style.height = "250px"
            
            # 350x250
            #            ================================================== 
            if gmc_widget_width is "350" and gmc_widget_height is "350"
              gmc_widget.style.width = "350px"
              gmc_widget.style.height = "350px"
              
              # iFrame content
              gmc_widget.contentWindow.document.write "<div id=\"gmc_widget\">                <div class=\"gmc_widget_inner gmc_widget_inner_small\">                   <div class=\"gmc_logo\"></div>                   <div class=\"gmc_widget_text_content\"> " + JSON.stringify(data.site_info.widget_description).replace(/\"/g, "") + "                     <p style=\"color:#b7b7b7;font-size:12px;\">* up to $250,000</p>                     <div class=\"gmc_stat_box medium\"><div class=\"gmc_stat_number\">" + JSON.stringify(data.numbers.share_number).replace(/\"/g, "") + "</div><span>Actions taken to help every woman and every child lead healthy lives.</span></div>                   </div>                 </div>                 <footer class=\"gmc_footer_medium\">                   <div class=\"gmc_footer_buttons\">                     <a href=\"http://globalmomschallenge.com/relay\" target=\"_blank\" class=\"gmc_button gmc_todays_post\">                       <i class=\"icon icon-chat\"></i>                       See Today's Post</a>                     <a href=\"" + JSON.stringify(data.site_info.learn_more_url).replace(/\"/g, "") + "\" target=\"_blank\" class=\"gmc_button gmc_learn_more\">                       " + JSON.stringify(data.site_info.learn_more_text).replace(/\"/g, "") + "</a>                     <div style=\"clear:both;\"></div>                   </div>                 </footer>                 </div>"
            
            # 300x600
            #            ================================================== 
            if gmc_widget_width is "300" and gmc_widget_height is "600"
              gmc_widget.style.width = "300px"
              gmc_widget.style.height = "600px"
              
              # iFrame content
              gmc_widget.contentWindow.document.write "<div id=\"gmc_widget\">                <header class=\"gmc_header_medium\"></header>                 <div class=\"gmc_widget_inner\">                   <div class=\"gmc_logo\"></div>                   <div class=\"gmc_widget_text_content\"> " + JSON.stringify(data.site_info.widget_description).replace(/\"/g, "") + "                     <div class=\"gmc_stat_box medium\"><div class=\"gmc_stat_number\">" + JSON.stringify(data.numbers.share_number).replace(/\"/g, "") + "</div><span>Actions taken to help every woman and every child lead healthy lives.</span></div>                   </div>                 </div>                 <footer class=\"gmc_footer_medium\">                   <div class=\"gmc_footer_buttons\">                     <a href=\"http://globalmomschallenge.com/relay\" target=\"_blank\" class=\"gmc_button gmc_todays_post\">                       <i class=\"icon icon-chat\"></i>                       See Today's Post</a>                     <a href=\"" + JSON.stringify(data.site_info.learn_more_url).replace(/\"/g, "") + "\" target=\"_blank\" class=\"gmc_button gmc_learn_more\">                       <i class=\"icon icon-info\"></i>                       " + JSON.stringify(data.site_info.learn_more_text).replace(/\"/g, "") + "</a>                     <div style=\"clear:both;\"></div>                   </div>                   <div class=\"gmc_notice\">* up to $250,000</div>                 </footer>                 </div>"
            
            # 300x1050
            #            ================================================== 
            if gmc_widget_width is "300" and gmc_widget_height is "1050"
              gmc_widget.style.width = "300px"
              gmc_widget.style.height = "1050px"
              
              # iFrame content
              gmc_widget.contentWindow.document.write "<div id=\"gmc_widget\">                <header class=\"gmc_header_large\"></header>                 <div class=\"gmc_widget_inner\">                   <img alt=\"GMC Logo\" src=\"data:image/png;base64,\"/>                   <div class=\"gmc_widget_text_content\"> " + JSON.stringify(data.site_info.widget_description).replace(/\"/g, "") + "                     <div class=\"gmc_stat_box\"><div class=\"gmc_stat_number\">" + JSON.stringify(data.numbers.share_number).replace(/\"/g, "") + "</div><span>Actions taken to help every woman and every child lead healthy lives.</span></div>                   </div>                 </div>                 <footer>                   <a href=\"http://globalmomschallenge.com/relay\" target=\"_blank\" class=\"gmc_button gmc_todays_post\">                     <i class=\"icon icon-chat\"></i>                     See Today's Post</a>                   <a href=\"" + JSON.stringify(data.site_info.learn_more_url).replace(/\"/g, "") + "\" target=\"_blank\" class=\"gmc_button gmc_learn_more\">                     <i class=\"icon icon-info\"></i>                     " + JSON.stringify(data.site_info.learn_more_text).replace(/\"/g, "") + "</a>                   <div class=\"gmc_site_link\"><a href=\"http://globalmomschallenge.org\" target=\"_blank\">GLOBALMOMSCHALLENGE.ORG</a></div>                   <div class=\"gmc_notice\">* up to $250,000</div>                 </footer>                 </div>"
            
            # set widget innerHTML to the HTML string
            # gmc_widget.innerHTML = gmc_widget_html;
            
            # Console log some data
            if debug
              console.log "-------------------------------------------------------------"
              console.log "data: ", data
          else
            
            # We reached our target server, but it returned an error
            gmc_widget.innerHTML = "Error"
          return

        request.onerror = ->

        
        # There was a connection error of some sort
        request.send()
      
      # DEBUG
      if debug
        console.log "-------------------------------------------------------------"
        console.log "script: ", script
        console.log "script.getAttribute(\"data-size\"): ", script.getAttribute("data-size")
    return
catch e
  console.log "There was an error: ", e